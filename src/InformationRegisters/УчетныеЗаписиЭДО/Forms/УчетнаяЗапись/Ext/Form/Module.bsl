...
Процедура ВыполнитьСохранениеДанныхВСервисахПослеПолученияВсехСертификатов(ДанныеСертификатов, Этапы) Экспорт
	...
	ИначеЕсли Запись.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
		...
		Если Этапы.ИзмененСоставСертификатов ИЛИ Этапы.ИзмененыРегистрационныеДанные Тогда
			...
			КонецЕсли;
			ПараметрыРегистрации.СертификатКриптографии = ДанныеСертификатов.СамыйНовый;
			...
			Оповещение = Новый ОписаниеОповещения("ПослеОтправкиРегистрационногоПакетаВСервис1СЭДО", ЭтотОбъект, Контекст);
			КонтекстДиагностики = ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики();
			
			Если ЗначениеЗаполнено(ДанныеСертификатов.СамыйНовый) Тогда
	...
	Если Этапы.ИзмененыНастройкиУведомлений Тогда
		...
		КлючСинхронизации.ВыбранныйСертификат = ДанныеСертификатов.СамыйНовый;
...
Функция РезультатПроверкиПередДобавлениемСертификата(Знач Сертификат, Знач Организация)
	...
	Возврат УчетныеЗаписиЭДОСлужебный.ПроверитьСертификатДляУчетнойЗаписи(Сертификат, Организация);
...
Процедура ДобавитьСертификатыУчетныхЗаписейЭДО(Знач ИдентификаторЭДО, Знач Сертификаты)
	...
	УчетныеЗаписиЭДОСлужебный.ЗаписатьСертификатыУчетнойЗаписи(ИдентификаторЭДО, Сертификаты);
...
Процедура ПослеПолученияДоступныхСертификатов(Результат, Контекст) Экспорт
	...
	ДанныеСертификатовИСамыйНовый = ДанныеСертификатовИСамыйНовый(Контекст.СписокСертификатов, Результат);
	
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, ДанныеСертификатовИСамыйНовый);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДанныеСертификатовИСамыйНовый(Знач СписокСертификатов, ДоступныеСертификаты)
	
	Результат = Новый Структура("СамыйНовый, Список, ОтборСертификатов");
	...
	Если ОтборСертификатов.Количество() > 0 Тогда
		Результат.СамыйНовый = ОтборСертификатов[0];
	КонецЕсли;
