...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ВыбранныйЦвет) Тогда
		ЭлементЦвета = Элементы[ВыбранныйЦвет];
		ЦветНажатие(ЭлементЦвета);
	КонецЕсли;
...
Процедура ЦветНажатие(Элемент)
	...
	ВыбранныйЦвет = Элемент.Имя;
	
	Для Каждого ДекорацияЦвет Из Элементы.ГруппаЦветаПоказателя.ПодчиненныеЭлементы Цикл
		
		Если ДекорацияЦвет = Элемент Тогда
			ДекорацияЦвет.Рамка = Новый Рамка(ТипРамкиЭлементаУправления.Одинарная, 5);
		Иначе
			ДекорацияЦвет.Рамка = Новый Рамка(ТипРамкиЭлементаУправления.БезРамки, 0);
		КонецЕсли;
