...
Процедура ОбновитьДанные()
	...
	Пока Выборка.Следующий() Цикл
		...
		Элементы["Выручка"+НомерМесяца].Заголовок = ФорматированнаяСтрокаДляДиаграммы(Выручка, МаксВыручка, Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Зеленый"));
		Элементы["Себестоимость"+НомерМесяца].Заголовок = ФорматированнаяСтрокаДляДиаграммы(Себестоимость, МаксСебестоимость, Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Синий"));
		Элементы["ВаловаяПрибыль"+НомерМесяца].Заголовок = ФорматированнаяСтрокаДляДиаграммы(ВаловаяПрибыль, МаксВаловаяПрибыль, Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Коралловый"));
		Элементы["Расходы"+НомерМесяца].Заголовок = ФорматированнаяСтрокаДляДиаграммы(Расходы, МаксРасходы, Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Оранжевый"));
		Элементы["Прибыль"+НомерМесяца].Заголовок = ФорматированнаяСтрокаДляДиаграммы(Прибыль, МаксПрибыль, Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Пурпурный"));
...
Функция ФорматированнаяСтрокаДляДиаграммы(ТекущееЗначение, МаксЗначение, ЦветЗначения)
	...
	ЦветПустогоЗначения = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Светло-серый");
	...
	ЭлементПредставлениеЗначения.Строка = Формат(ТекущееЗначение, НСтр("ru = 'ЧДЦ=2; ЧРГ='' ''; ЧН=0,00; ЧГ=3,0'"));
	Если ТекущееЗначение < 0 Тогда
		ЭлементПредставлениеЗначения.ЦветТекста = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Красный");
	Иначе
		ЭлементПредставлениеЗначения.ЦветТекста = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Серый");
	КонецЕсли;
