...
Процедура ДенежныеСредстваПодробнееНажатие(Элемент)
	...
	КомпоновщикНастроек = ОтчетыУНФВызовСервера.ПолучитьПереопределенныйКомпоновщикНастроек(СвойстваОтчета, ПараметрыИОтборы);
...
&НаКлиенте
Процедура ДекорацияУдалениеМонитораРуководителяЗакрытьНажатие(Элемент)
	
	Элементы.ГруппаУдалениеМонитораРуководителя.Видимость = Ложь;
...
Процедура ОбновитьВиджетДенежныеСредства()
	...
	Серия.Цвет = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Оранжевый");
	...
	Элементы.ДекорацияКассаОстаток.Заголовок = ?(ТекущийОстаток = 0, "—", Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ТекущийОстаток));
	...
	ИначеЕсли ИзменениеПроцент < 0 Тогда
		...
		Элементы.ДекорацияКассаПроцент.ЦветТекста = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Красный");
	ИначеЕсли ИзменениеПроцент > 0 Тогда
		Элементы.ДекорацияКассаПроцент.Видимость = Истина;
		Элементы.ДекорацияКассаПроцент.Заголовок = "+" + Формат(ИзменениеПроцент, "ЧДЦ=") + "%";
		Элементы.ДекорацияКассаПроцент.ЦветТекста = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Зеленый");
	КонецЕсли;
	...
	Элементы.ДекорацияСчетаОстаток.Заголовок = ?(ТекущийОстаток = 0, "—", Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ТекущийОстаток));
	ИзменениеПроцент = ?(ОстатокВчера = 0, 0, Окр((ТекущийОстаток - ОстатокВчера) / ОстатокВчера * 100));
	Если ИзменениеПроцент = 0 Тогда
	...
	ИначеЕсли ИзменениеПроцент < 0 Тогда
		...
		Элементы.ДекорацияСчетаПроцент.ЦветТекста = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Красный");
	ИначеЕсли ИзменениеПроцент > 0 Тогда
		Элементы.ДекорацияСчетаПроцент.Видимость = Истина;
		Элементы.ДекорацияСчетаПроцент.Заголовок = "+" + Формат(ИзменениеПроцент, "ЧДЦ=") + "%";
		Элементы.ДекорацияСчетаПроцент.ЦветТекста = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Зеленый");
	КонецЕсли;
...
Процедура ОбновитьВиджетПрибыльУбыток()
	...
	Если ВыборкаОбщийИтог.Следующий() Тогда
		Элементы.ДекорацияДоходыИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщийИтог.Доходы);
		Элементы.ДекорацияРасходыИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщийИтог.Расходы);
		Элементы.ДекорацияПрибыльИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщийИтог.Прибыль);
	Иначе
...
Процедура ОбновитьВиджетДебиторы()
	...
	Если ВыборкаОбщиеИтоги.Следующий() Тогда
		...
		Элементы.ДекорацияДебиторыЗадолженностьИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщиеИтоги.СуммаДолга);
		Элементы.ДекорацияДебиторыПросроченоИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщиеИтоги.СуммаПросрочено);
	Иначе
...
Процедура ОбновитьВиджетКредиторы()
	...
	Если ВыборкаОбщиеИтоги.Следующий() Тогда
		...
		Элементы.ДекорацияКредиторыЗадолженностьИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщиеИтоги.СуммаДолга);
		Элементы.ДекорацияКредиторыПросроченоИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщиеИтоги.СуммаПросрочено);
	Иначе
...
Процедура ОбновитьВиджетПродажи()
	...
	Если ВыборкаОбщийИтог.Следующий() Тогда
		Элементы.ДекорацияПродажиИтог.Заголовок = Обработки.УдалитьМониторыРуководителя.СформироватьЗаголовок(ВыборкаОбщийИтог.Сумма);
	Иначе
	...
	Серия.Цвет = Обработки.УдалитьМониторыРуководителя.ЦветДляМониторов("Темно-зеленый");
