...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СтруктураКаталога", СтруктураКаталога) Тогда
		...
		Товары = ПолучитьИзВременногоХранилища(СтруктураКаталога.ТоварыКаталога);
		
		НастройкаОтправки = НастройкиЭДОКлиентСервер.НоваяНастройкаОтправки();
		НастройкаОтправки.ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(
			Перечисления.ТипыДокументовЭДО.КаталогТоваров);
		НастройкаОтправки.Формат = ЭлектронныеДокументыЭДО.ПоддерживаемыеФорматы().CML208;
		НастройкаОтправки.Отправитель = СтруктураКаталога.Организация;
		ОписаниеСообщения = ЭлектронныеДокументыЭДО.ОписаниеСообщенияОтправителя(Справочники.НастройкиЭДО.ПустаяСсылка(),
			НастройкаОтправки, Товары);

		НоваяСтрока = ТаблицаДанных.Добавить();
		НоваяСтрока.ПолноеИмяФайла = ОписаниеСообщения.Данные.Документ.ИмяФайла;
		НоваяСтрока.ИмяФайла = ОписаниеСообщения.Данные.Документ.ИмяФайла;// TO DO Проверить
		ДвоичныеДанныеФайла = ОписаниеСообщения.Данные.Документ.ДвоичныеДанные;
		
		ДвоичныеДанныеПакета = Неопределено; //TO DO: Получить двоичные данные пакета

		НоваяСтрока.АдресХранилищаПакета = ПоместитьВоВременноеХранилище(ДвоичныеДанныеПакета, УникальныйИдентификатор);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Ошибки) Тогда
		...
		СтруктураОшибки = Новый Структура("ОписаниеОбъектаУчета, ОшибкиДанных", ИнтеграцияЭДОВызовСервера.ОписаниеОбъектаУчета(ДанныеОшибки.ОбъектУчета),
			ДанныеОшибки.Ошибки);
