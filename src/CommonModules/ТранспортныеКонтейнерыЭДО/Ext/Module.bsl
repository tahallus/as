...
Процедура ОбработатьОшибкуЗагрузкиОбъекта(ДанныеОбъекта, КонтекстДиагностики, ТекстОшибки) Экспорт
...
// Устарела. Следует использовать ПараметрыКонтейнера для инициализации параметров и ЗаполнитьПараметрыКонтейнера для заполнения
...
// * Объект - Произвольный
Функция ПараметрыТранспортногоКонтейнера(ИдентификаторДокументооборота, ИдентификаторСообщения, ИдентификаторПакета,
	ДвоичныеДанные, СпособОбмена, ИдентификаторУчетнойЗаписи, КонтекстДиагностики)
	
	ПараметрыКонтейнера = ПараметрыКонтейнера(ИдентификаторДокументооборота,
		ИдентификаторСообщения,
		ИдентификаторПакета,
		СпособОбмена,
		ИдентификаторУчетнойЗаписи);
	
	ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанные, КонтекстДиагностики);
...
Функция ПараметрыКонтейнера(ИдентификаторДокументооборота, ИдентификаторСообщения, ИдентификаторПакета,
	СпособОбмена, ИдентификаторУчетнойЗаписи)
...
Процедура ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанные, КонтекстДиагностики)
	...
	КонецЕсли;
	Если Контрагент = Неопределено И ЗначениеЗаполнено(Карточка.ИННКонтрагента) Тогда
		Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП("Контрагенты", Карточка.ИННКонтрагента,
	...
	Если Не ЗначениеЗаполнено(Контрагент) И СоздаватьКонтрагентовАвтоматическиБЭД И ЗначениеЗаполнено(
		Карточка.ИННКонтрагента) Тогда
	...
	Если Не ЗначениеЗаполнено(Контрагент) И ЗначениеЗаполнено(Карточка.ИдентификаторОтправителя) Тогда
		...
		ИтоговыйЗапрос.УстановитьПараметр("ИдентификаторОрганизации", ПараметрыКонтейнера.Получатель);
		ИтоговыйЗапрос.УстановитьПараметр("ИдентификаторКонтрагента", Карточка.ИдентификаторОтправителя);
...
Функция ЗаполнитьТранспортныйКонтейнер(ПараметрыКонтейнера, Знач ТранспортныйКонтейнер = Неопределено)
	...
	Попытка
		Если ТранспортныйКонтейнер = Неопределено Тогда
			ТранспортныйКонтейнер = Документы.ТранспортныйКонтейнерЭДО.СоздатьДокумент();
		Иначе
			ТранспортныйКонтейнер = ТранспортныйКонтейнер.ПолучитьОбъект();
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыКонтейнера.Статус) Тогда
			Статус = ПараметрыКонтейнера.Статус;
		ИначеЕсли Не ЗначениеЗаполнено(ПараметрыКонтейнера.Контрагент)
			Или Не ЗначениеЗаполнено(ПараметрыКонтейнера.Организация)
			Или Не ЗначениеЗаполнено(ПараметрыКонтейнера.ДоговорКонтрагента)
			Или Не ЗначениеЗаполнено(ПараметрыКонтейнера.Получатель) Тогда
		...
		Если Не ЗначениеЗаполнено(ТранспортныйКонтейнер.Дата) Тогда
		...
		Для каждого ЭлементДанных Из ДанныеЗаполнения Цикл
			Если ЗначениеЗаполнено(ЭлементДанных.Значение)
				И ТранспортныйКонтейнер[ЭлементДанных.Ключ] <> ЭлементДанных.Значение Тогда
				ТранспортныйКонтейнер[ЭлементДанных.Ключ] = ЭлементДанных.Значение;
				Записывать = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если Записывать Тогда
			ТранспортныйКонтейнер.Записать();
		КонецЕсли;
	...
	Возврат ТранспортныйКонтейнер.Ссылка;
	
КонецФункции

Процедура ДобавитьПрисоединенныйФайлКонтейнера(Контейнер, ДвоичныеДанныеКонтейнера, ИмяФайла)
	...
	Для каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнеры Цикл
		...
		Попытка
			...
			Если (ЗначенияРеквизитовКонтейнера.Статус = Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный
				Или Не ЗначениеЗаполнено(ЗначенияРеквизитовКонтейнера.Получатель)) Тогда
...
Функция НовоеОписаниеДокументаКонтейнера()
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ТранспортныйКонтейнер) Тогда
			Если СинхронизацияЭДОКлиентСервер.ЭтоОбменЧерезОператора(ПараметрыКонтейнера.СпособОбменаЭД)
				...
				Если Не ТранспортныеКонтейнерыЭДОСлужебный.СформироватьПрисоединенныйФайлТранспортногоКонтейнераОператораЭДО(
					ТранспортныйКонтейнер, ДанныеОбъектов, КонтекстДиагностики) Тогда
					ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка при создании присоединенного файла транспортного контейнера: %1'"),
						ДанныеОбъекта.Представление);
					ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстОшибки,
						ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами);
					ТранспортныеКонтейнеры = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТранспортныйКонтейнер);
					ТранспортныеКонтейнерыЭДОСлужебный.ОтменитьРаспаковкуКонтейнеров(ТранспортныеКонтейнеры);
				КонецЕсли;
