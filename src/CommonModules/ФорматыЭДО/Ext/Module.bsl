...
Функция ЗапросФорматовПоУмолчанию(ИмяВременнойТаблицы = "", Отбор = Неопределено) Экспорт
	...
	ОписаниеЗапроса.Текст = 
		...
		|ВЫБРАТЬ
		|	СвязьВидовФорматов.ВидДокумента КАК ВидДокумента,
		|	МИНИМУМ(СвязьВидовФорматов.Приоритет) КАК Приоритет
...
// Инициализирует настройки формирования прикладного электронного документа.
//
// Возвращаемое значение:
//  Структура - параметры формирования:
//   * ПрикладнойТип - ОпределяемыйТип.ПрикладныеТипыЭлектронныхДокументовЭДО - прикладной тип документа.
//   * Отправитель - ОпределяемыйТип.Организация - отправитель документа.
//   * Получатель - ОпределяемыйТип.КонтрагентБЭД - получатель документа.
//
Функция НовыеНастройкиФормированияПрикладногоДокумента()
	
	Параметры = Новый Структура;
	Параметры.Вставить("ПрикладнойТип", Метаданные.ОпределяемыеТипы.ПрикладныеТипыЭлектронныхДокументовЭДО.Тип.ПривестиЗначение());
	Параметры.Вставить("Отправитель", Метаданные.ОпределяемыеТипы.Организация.Тип.ПривестиЗначение());
	Параметры.Вставить("Получатель", Метаданные.ОпределяемыеТипы.КонтрагентБЭД.Тип.ПривестиЗначение());
...
Функция СформироватьДанныеОсновногоТитулаПоОбъектуУчета(ОбъектУчета, Настройки, Данные) Экспорт
	...
	Если Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		СчетаФактура = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДанныеДокумента,
	...
	Иначе
		Если ЗначениеЗаполнено(СчетаФактура) Тогда
			ЗначенияДополнительныхПолей = ДополнитьДокументДаннымиДополнительныхПолей(СчетаФактура, Настройки, ДанныеДокумента,
				Данные.ЗначенияДополнительныхПолей);
		Иначе
			ЗначенияДополнительныхПолей = ДополнитьДокументДаннымиДополнительныхПолей(ОбъектУчета, Настройки, ДанныеДокумента,
				Данные.ЗначенияДополнительныхПолей);
		КонецЕсли;
	КонецЕсли;
...
// Параметры:
//  ОбъектУчета - ОпределяемыеТипы.ОснованияЭлектронныхДокументовЭДО
//  Настройки - см. НовыеНастройкиФормированияПрикладногоДокумента
//  Данные - см. НовыеДанныеДляФормированияПрикладногоДокумента
//
// Возвращаемое значение:
//  См. НовыйРезультатФормированияДокументаПоУчету
//
Функция СформироватьДанныеПрикладногоДокумента(Знач ОбъектУчета, Знач Настройки, Знач Данные) Экспорт
...
Функция ДанныеЭлектронногоДокумента(ПараметрыПолученияДанных) Экспорт
	...
	Если СтруктураРазбора = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРазбора) Тогда
			...
			СтруктураРазбора.Свойство("НомерДокументаОтправителя", ОписаниеФайла.НомерДокумента);
			СтруктураРазбора.Свойство("ДатаДокументаОтправителя", ОписаниеФайла.ДатаДокумента);
	...
	Если СтруктураРазбора = Неопределено Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Неизвестный тип электронного документа при чтении данных из файла %1.'"), ПараметрыПолученияДанных.ОсновнойФайл.ИмяФайла);
	...
	Если ТипЗнч(СтруктураРазбора) = Тип("Структура") Тогда
		...
		Если СтруктураРазбора.Свойство("ТипДокументаДляОтраженияВУчете") Тогда
				ОписаниеФайла = СтруктураРазбора;
				...
				Если ДанныеCML <> Неопределено Тогда
					...
					СтруктураРазбора = ДанныеЭлектронногоДокументаОсновныхТипов(ПараметрыПолученияДанныхCML);
					ОписаниеФайла.Свойство("ТипДокумента", СтруктураРазбора.Содержание.ТипДокумента);							
				КонецЕсли;
...
Функция ДополнитьДокументДаннымиДополнительныхПолей(ОбъектУчета, ПараметрыПолучения, ДеревоДанных, КэшДополнительныхДанных)
	...
	НастройкаЗаполнения = НастройкиЭДО.НастройкаЗаполненияДополнительныхПолей(КлючНастройкиДопПолей, ПараметрыПолучения.Формат);
	...
	ПараметрыЗаполнения.Вставить("СсылкаНаОбъект", ОбъектУчета);
	ПараметрыЗаполнения.Вставить("КлючНастройки", КлючНастроекОтправки);
...
Функция ВычислитьЗначенияДополнительныхПолейШапки(ПараметрыЗаполнения, ОшибкиВычисления)
	...
	Для Каждого НастройкаПоФормуле Из МассивНастроекПоФормуле Цикл
		...
		ВерсияМеханикиРасчета = "1.0";
		Если НастройкаПоФормуле.Версия = НастройкиЭДОКлиентСервер.ВерсияНастроекДополнительныхПолейПоФормуле() Тогда
			ВерсияМеханикиРасчета = "2.0";
		КонецЕсли;
		
		Попытка 
			ЗначениеФормулы = ОбщегоНазначенияБЭД.РезультатВычисленияФормулы(Формула, ДанныеЗначенийОперандов,
				ВерсияМеханикиРасчета);
...
//
Процедура ДополнитьИнформациюОТовареДаннымиДляСопоставления(ИнформацияОТоваре, ЭтоСчетФактураНаАванс = Ложь, СодержитИзменяемыеДанные = Ложь)
	...
	Для Каждого СтрокаТовара Из ИнформацияОТоваре Цикл
		...
		ИначеЕсли Не СтрокаТовара.Сопоставление.Свойство("Идентификатор") Тогда
...
Функция НовыеДанныеДляСопоставленияТоваровПоДаннымНоменклатуры(Знач Идентификатор, Знач Наименование, Знач Характеристика, Знач ЕдиницаИзмерения)
	...
	Если Не ЗначениеЗаполнено(Идентификатор) Тогда
		ИдентификаторНоменклатуры = СопоставлениеНоменклатурыКонтрагентов.ИдентификаторТовараПоСтроке(Наименование, 100);
		Если ЗначениеЗаполнено(Характеристика) Тогда
			ИдентификаторХарактеристики = СопоставлениеНоменклатурыКонтрагентов.ИдентификаторТовараПоСтроке(Характеристика, 100);
		Иначе
			ИдентификаторХарактеристики = "";
		КонецЕсли;
		Если ЗначениеЗаполнено(ЕдиницаИзмерения) Тогда
			ИдентификаторУпаковки = СопоставлениеНоменклатурыКонтрагентов.ИдентификаторТовараПоСтроке(ЕдиницаИзмерения, 100);
		Иначе
...
Функция ЭтоФорматПространства(ПространствоФорматов, Формат)
