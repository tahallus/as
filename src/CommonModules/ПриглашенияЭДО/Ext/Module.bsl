...
Функция СоздатьПриглашениеКОтправкеОператоруЭДО(НатуральныеКлючиПриглашения, Контрагент = Неопределено,
	ТекстПриглашения = Неопределено, ОписаниеОшибки = Неопределено) Экспорт

	Возврат ПриглашенияЭДОСлужебный.СоздатьПриглашениеКОтправкеОператоруЭДО(НатуральныеКлючиПриглашения,
		Контрагент, ТекстПриглашения, ОписаниеОшибки);
...
// Возвращает вид ошибки, описывающий ситуацию, когда отключена настройка автоматического создания контрагентов.
// 
// Возвращаемое значение:
// 	См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиАвтоматическоеСозданиеКонтрагентаНевозможно()
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "АвтоматическоеСозданиеКонтрагентаНевозможно";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Автоматическое создание контрагента невозможно'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Выключена опция ""Создавать контрагентов автоматически""'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Заполните контрагента и <a href = ""примите"">примите</a> приглашение'");
	ВидОшибки.ОбработчикиНажатия.Вставить("примите", "ПриглашенияЭДОКлиент.ОткрытьПриглашения");
...
#Область РегистрыСведений_ПриглашенияКОбменуЭлектроннымиДокументами_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Процедура = "РегистрыСведений.ПриглашенияКОбменуЭлектроннымиДокументами.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("26929902-0a33-455c-a620-648352afacb1");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "РегистрыСведений.ПриглашенияКОбменуЭлектроннымиДокументами.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.УдалитьПриглашенияКОбменуЭлектроннымиДокументами";
	Обработчик.БлокируемыеОбъекты = "РегистрСведений.УдалитьПриглашенияКОбменуЭлектроннымиДокументами";
	Обработчик.ИзменяемыеОбъекты = "РегистрСведений.ПриглашенияКОбменуЭлектроннымиДокументами," +
		"РегистрСведений.УдалитьПриглашенияКОбменуЭлектроннымиДокументами";
...
Процедура ПроверитьНеполученныеПриглашения(КлючиСинхронизации, КонтекстДиагностики, ПараметрыОсновногоЗапросаПриглашений,
	...
	Если РезультатОбработки.ДанныеПоУчетнымЗаписям <> Неопределено Тогда
		Для каждого ДанныеПоУчетнойЗаписи Из РезультатОбработки.ДанныеПоУчетнымЗаписям Цикл
			...
			ДополнительныеДанные.Вставить("ПроверочнаяДатаПолученияЭД", ДанныеПоУчетнойЗаписи.ДатаПервогоНеполученного);
