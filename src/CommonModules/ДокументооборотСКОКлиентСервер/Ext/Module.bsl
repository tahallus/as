...
Функция ПроверитьИННВладельцаЭП(Форма) Экспорт
	...
	Если ПустаяСтрока(Форма.ВладелецЭЦПИНН) Тогда
...
Функция IdentKind(ДокументЗаявление) Экспорт
	...
	УдаленнаяАутентификация = НЕ ДокументЗаявление.ТребуетсяВстречаСПартнером И ДокументЗаявление.ПодписатьЭП;
	Возврат Число(УдаленнаяАутентификация);
...
Функция ИННИзСертификата(Сертификат, ОбрезатьНули = Истина, ЭтоЮЛ = Неопределено) Экспорт
	...
	Имена.Добавить("_1_2_643_100_4"); // ИНН организации по-новому
	Имена.Добавить("OID1_2_643_100_4"); // ИНН организации по-новому
	Имена.Добавить("INNLE"); // ИНН ЮЛ по-новому
	Имена.Добавить("OID1_2_643_3_131_1_1"); // ИНН ИП по-новому ИЛИ ИНН ЮЛ по-старому 
	Имена.Добавить("INN");
	Имена.Добавить("ИНН");
	...
	Для каждого Имя Из Имена Цикл
		...
		Найден   = ИНННайден(Значение, ЭтоЮЛ);
		Если Найден Тогда
			ИНН = Значение;
			Прервать;
		КонецЕсли;
...
Функция ИНННайден(Значение, ЭтоЮЛ)
	
	Если НЕ ЗначениеЗаполнено(Значение) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ЭтоЮЛ = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;

	Длина   = СтрДлина(Значение);
	Префикс = Лев(Значение, 2); 
	
	ЭтоИННЮЛ = Префикс = "00" ИЛИ Длина = 10;
	Если ЭтоЮЛ = Истина И ЭтоИННЮЛ Тогда
		Возврат Истина;
	КонецЕсли;
	
	ЭтоИННФЛ = Префикс <> "00" И Длина = 12;
	Если ЭтоЮЛ = Ложь И ЭтоИННФЛ Тогда
		Возврат Истина;
	КонецЕсли;
