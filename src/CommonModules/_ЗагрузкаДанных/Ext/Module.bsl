...
Функция ОчиститьНомерТС(Знач НомерТС) Экспорт
	
	НомерТС = СокрЛП(НомерТС);
	 
	НомерТС = СтрЗаменить(НомерТС, "RUS"	, "" );
	НомерТС = СтрЗаменить(НомерТС, "\"		, "/");
	НомерТС = СтрЗаменить(НомерТС, "////"	, "/");
	НомерТС = СтрЗаменить(НомерТС, "///"	, "/");
	НомерТС = СтрЗаменить(НомерТС, "//"		, "/");
	НомерТС = СтрЗаменить(НомерТС, "   "	, "/");
	...
	НомерТС = ?(Лев (НомерТС, 1) = "/", Сред(НомерТС, 2)					, НомерТС);
	НомерТС = ?(Прав(НомерТС, 1) = "/", Лев (НомерТС, СтрДлина(НомерТС)-1 )	, НомерТС);
