...
Функция ДвоичныеДанныеТранспортногоКонтейнера(ДанныеОбъектов, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектов Цикл
		...
		// Формируем card.xml
		СформироватьКарточку(ДанныеОбъекта, АдресПодкаталога, Ошибки, ДанныеОбъекта, КонтекстДиагностики);
...
Функция СформироватьТранспортнуюИнформациюCML(ДеревоДанных, Ошибки, ОписаниеОбъекта, КонтекстДиагностики)
	...
	Исключение
		...
		ДобавитьОшибкуФормированияТранспортногоКонтейнера(КонтекстДиагностики, ИнформацияОбОшибке(),
			ОписаниеОбъекта.Ссылка, ТекстСообщения);
...
Процедура ПоместитьАрхивВКонтейнер(ТранспортныйКонтейнер, ДвоичныеДанные, ИмяФайлаБезРасширения, Расширение)
	...
	Попытка
		...
		ПрисоединенныеФайлы = ТранспортныеКонтейнерыЭДО.ПрисоединенныеФайлыКонтейнеров(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТранспортныйКонтейнер));

		Если ПрисоединенныеФайлы[ТранспортныйКонтейнер] <> Неопределено
			И ПрисоединенныеФайлы[ТранспортныйКонтейнер].Количество() > 0 Тогда
