...
Процедура СформироватьПлатежкуВНалоговую(Данные, ОбластьМакета, ТабличныйДокумент, ВидВзаиморасчетов, СуммаНалога) 
	...
	Если ТипЗнч(Данные) = Тип("ДокументСсылка.НачислениеНалогов") Тогда
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ВТ_КонтактнаяИнформация.Ссылка КАК Ссылка,
		|	ВТ_КонтактнаяИнформация.Представление КАК Представление
		|ИЗ
		|	Справочник.Организации.КонтактнаяИнформация КАК ВТ_КонтактнаяИнформация
		|ГДЕ
		|	ВТ_КонтактнаяИнформация.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ЮрАдресОрганизации)
		|	И ВТ_КонтактнаяИнформация.Ссылка = &Организация
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВТ_КонтактнаяИнформация.ДействуетС УБЫВ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
