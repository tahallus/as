...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "НЕДЕЛЯ", Периодичность);
	Запрос.УстановитьПараметр("НачалоПериода", НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", КонецПериода);
	Запрос.УстановитьПараметр("ТаблицаСреднийОбъем", ИсправитьТипыТаблицы(Таблица));
	...
	СтруктураПараметры = Новый Структура("ТаблицаСреднийОбъем", ИсправитьТипыТаблицы(ТаблицаИсточник));
...
// 
Функция ИсправитьТипыТаблицы(Таблица)
	
	Результат = Новый ТаблицаЗначений;
	Для каждого Колонка Из Таблица.Колонки Цикл
		НовыйТип = Новый ОписаниеТипов(Колонка.ТипЗначения, , "Null");
		Если НовыйТип.Типы().Количество() = 0 Тогда
			Продолжить;	
		КонецЕсли;
		НоваяКолонка = Результат.Колонки.Добавить(Колонка.Имя, НовыйТип);
	КонецЦикла;                                                          
